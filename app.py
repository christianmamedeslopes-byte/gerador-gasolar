...
# 2. DESIGN DA INTERFACE (CSS CORRIGIDO)
# O parâmetro correto é unsafe_allow_html
st.markdown("""
    <style>
...
# 4. SIDEBAR
with st.sidebar:
    st.markdown("<h1 style='color: #0f172a;'>M e Lopes</h1>", unsafe_allow_html=True)
...
<head>
    <style>
        ...
        .card {{ background: #f8fafc; padding: 15px; border: 1px solid #e2e8f0; border-radius: 8px; margin: 20px 0; }}
        table {{ width: 100%; border-collapse: collapse; }}
        th {{ text-align: left; padding: 10px; font-size: 8pt; color: #64748b; text-transform: uppercase; border-bottom: 2px solid #e2e8f0; }}
        td {{ padding: 10px; border-bottom: 1px solid #f1f5f9; font-size: 10pt; }}
        .footer {{ margin-top: 40px; text-align: center; color: #94a3b8; font-size: 8pt; border-top: 1px solid #f1f5f9; padding-top: 20px; }}
    </style>
</head>
<body>
    <div class="header">
        <div style="font-size: 20pt; font-weight: 800; color: #0f172a;">M e Lopes</div>
        <div style="text-align: right; color: #64748b;">{datetime.now().strftime('%d/%m/%Y')}</div>
    </div>
    <h2>Caixa de Obra: {cliente_selecionado}</h2>
    <div class="card">
        <strong>RESUMO:</strong> Gasto R$ {total:,.2f} | Adiantamento R$ {adiantamento:,.2f} | <strong>{status}: R$ {abs(saldo):,.2f}</strong>
    </div>
    {tabela_html}
    <div class="footer">M e Lopes, assessoria em tecnologia</div>
</body>
</html>
"""
